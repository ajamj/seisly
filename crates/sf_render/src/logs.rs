...
impl LogRenderer {
    pub fn generate_strip_mesh(traj: &Trajectory, curve: &Curve, width: f32) -> Mesh {
        ...
        for i in 0..n {
            let md_norm = if n > 1 { i as f64 / (n - 1) as f64 } else { 0.0 };
            let md = md_min + md_norm * (md_max - md_min);
            
            let station = interpolate_station(traj, md);
            
            // Width offset (just in X for now)
            let v_left = [station.x as f32 - width / 2.0, station.y as f32, station.z as f32];
            let v_right = [station.x as f32 + width / 2.0, station.y as f32, station.z as f32];
            
            let val = curve.values[i];
            let norm_val = if val == curve.null_value { 0.0 } else { (val - min_val) / range };
