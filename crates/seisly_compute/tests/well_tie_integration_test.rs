...
fn test_well_tie_full_workflow() {
    ...
    assert!(
        min_depth <= 0.0,
        "Min depth should be <= 0, got {}",
        min_depth
    );
    assert!(
        max_depth >= 1000.0,
        "Max depth should be >= 1000, got {}",
        max_depth
    );
    ...
    for i in 1..tie.time_depth_pairs.len() {
        assert!(
            ...
            i,
            tie.time_depth_pairs[i].twt,
            i - 1,
            tie.time_depth_pairs[i - 1].twt
        );
        assert!(
            tie.time_depth_pairs[i].depth_md > tie.time_depth_pairs[i - 1].depth_md,
            "Depth should increase: pair[{}] = {}m, pair[{}] = {}m",
            i,
            tie.time_depth_pairs[i].depth_md,
            i - 1,
            tie.time_depth_pairs[i - 1].depth_md
        );
    }
...
fn test_well_without_logs_error() {
    let well = Well::new("Empty Well".to_string(), "EW".to_string(), 0.0, 0.0, 0.0);
    let engine = WellTieEngine::new(2000.0, 0.5);
...
fn test_well_tie_shallow_vs_deep() {
    ...
    let mut shallow_well = Well::new("Shallow Well".to_string(), "SW".to_string(), 0.0, 0.0, 0.0);
    let shallow_depths: Vec<f32> = (0..51).map(|i| i as f32 * 10.0).collect();
    let shallow_values: Vec<f32> = vec![50.0; 51];
    shallow_well.add_log(
        "GR".to_string(),
        "GAPI".to_string(),
        shallow_values,
        shallow_depths,
    ...
    let mut deep_well = Well::new("Deep Well".to_string(), "DW".to_string(), 0.0, 0.0, 0.0);
    ...
    assert!(
        (shallow_twt_500 - deep_twt_500).abs() < 1.0,
        "TWT at 500m should be same: shallow={}, deep={}",
        shallow_twt_500,
        deep_twt_500
    );
}
...
fn test_velocity_model_edge_cases() {
    ...
    assert!(
        twt_zero.abs() < 1e-10,
        "TWT at depth 0 should be 0, got {}",
        twt_zero
    );
    ...
    assert!(
        depth_zero.abs() < 1e-10,
        "Depth at TWT 0 should be 0, got {}",
        depth_zero
    );
    ...
    assert!(
        twt_shallow > 0.0 && twt_shallow < 10.0,
        "TWT at 1m should be small positive, got {}",
        twt_shallow
    );
    ...
    assert!(
        twt_deep > 2000.0,
        "TWT at 5000m should be large, got {}",
        twt_deep
    );
}
...
fn test_time_depth_pair_consistency() {
    ...
    for i in 1..tie.time_depth_pairs.len() {
        ...
        assert!(
            ...
            "Depth interval should be 10m, got {}",
            depth_interval
        );
    }
}
