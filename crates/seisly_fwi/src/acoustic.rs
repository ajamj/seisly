        ...
        for t in 0..nt {
            ...
            for iz in 1..nz - 1 {
                for ix in 1..nx - 1 {
                    ...
                    let d2u_dx2 = (u_curr[(iz, ix + 1)] - 2.0 * u_curr[(iz, ix)]
                        + u_curr[(iz, ix - 1)])
                        / dx2;
                    let d2u_dz2 = (u_curr[(iz + 1, ix)] - 2.0 * u_curr[(iz, ix)]
                        + u_curr[(iz - 1, ix)])
                        / dz2;
...
impl AcousticFWI {
    ...
    pub fn misfit(&self, velocity: &Array2<f32>, source: &Source, nt: usize) -> f32 {
        ...
        let new_solver = AcousticWaveSolver::new(
            velocity.clone(),
            self.solver.dt,
            self.solver.dx,
            self.solver.dz,
    ...
    pub fn gradient(&self, velocity: &Array2<f32>, source: &Source, nt: usize) -> Array2<f32> {
        ...
        for iz in 0..velocity.nrows() {
            for ix in 0..velocity.ncols() {
                ...
                let mut sum = 0.0f32;
                for t in 0..nt {
                    sum += wavefield_fwd[[t, iz, ix]] * wavefield_fwd[[t, iz, ix]];
                    // Simplified
                }
