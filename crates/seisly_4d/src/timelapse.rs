...
impl TimeLapseAnalysis {
    ...
    pub fn nrms(&self) -> f32 {
        ...
        let rms_base: f32 = (self.base_survey.iter().map(|x| x * x).sum::<f32>()
            / self.base_survey.len() as f32)
            .sqrt();
        let rms_monitor: f32 = (self.monitor_survey.iter().map(|x| x * x).sum::<f32>()
            / self.monitor_survey.len() as f32)
            .sqrt();
