...
use seisly_qi::{
    AvoAnalysis, AvoClass, BrightSpot, Dhi, FluidFactor, Gassmann, PoissonsRatio, VpVsRatio,
};
...
fn test_gassmann_fluid_substitution() {
    ...
    let vp_hc =
        Gassmann::vp_after_substitution(vp_brine, vs_brine, rho_brine, k_brine, k_hc, porosity);
...
fn test_dhi_analysis() {
    ...
    assert!(
        prob > 0.8,
        "Class 3 AVO should have high bright spot probability"
    );
}
