...
This project uses metaswarm for multi-agent orchestration and structured development workflows.

### Command Shims (`.claude/commands/`)

| Command | Purpose |
|---------|---------|
| `/start-task` | Begin tracked work with checkpoint and TDD |
| `/prime` | Load project context and suggest next action |
| `/review-design` | Spawn 5 review agents (PM, Architect, Designer, Security, CTO) |
| `/self-reflect` | Extract learnings after PR merge or session |
| `/pr-shepherd` | Monitor PR through CI, review, merge |
| `/brainstorm` | Refine idea → design → review → plan |
...
# Per-crate coverage (see .coverage-thresholds.json)
cargo tarpaulin --workspace --fail-under 70
```

### Project Profile

Located at `.metaswarm/project-profile.json`. Contains detected stack, coverage config, and workflow settings.

### GSD Planning

Plans live in `.planning/phases/`. Each phase has:
- `{phase}-PLAN.md` — Executable work units with tasks, acceptance criteria, verification
- `{phase}-CONTEXT.md` — Implementation decisions from discuss-phase
- `{phase}-RESEARCH.md` — Technical research (if conducted)
- `{phase}-SUMMARY.md` — Completion summary (created on finish)
